...
draw.text((32, 22), 'Clock', fill = "BLUE", font = FONT_BIG)
draw.text((45, 40), 'Thing', fill = "BLUE",font = FONT_BIG)
draw.text((10, 60), 'demo', fill = "BLUE", font = FONT_SMALL)
...
def updateScreenClock():
    value= time.asctime()
    
    image = Image.new("RGB", (160,20), COLOR_FG)
    draw = ImageDraw.Draw(image)

    draw.text((0, 0), str(value)[10:20], fill = "BLACK", font = FONT_BIG)

    #LCD.LCD_ShowImage(image,0,120,w=160,h=18)
    LCD.display_window(image,0,50,160,70)
